# Golf Video Recording Plan (First-Person AR Glasses)

**Goal:** Capture first-person golf footage where **the ball and the club head are both fully and clearly in frame at address (setup)**.

## General rules
- **People: 6** (at least 1 left-handed — spread the lefty across both settings and several clubs, not just one clip); vary height, build, and swing speed.
...
- Cover the 6 people × 6 clubs × 4 balls variety here; change 2–3 bays, move the mat, different times of day, so clips don't all look alike.
- Keep natural mishits (thins, tops); **don't fake whiffs.**
- Empty-scene negatives: ball buckets, alignment sticks, yardage markers.

## Course / on the field (for real-world variety)
- At least a few clips per person; **putts mostly on a real green**, chipping and multi-ball practice on the practice green / short-game area (where you can drop several balls without blocking play).
- On the course, **use just one ball and one set of clubs**; only use the driver on the tee — don't switch among 4 balls / 6 clubs mid-round.
- Capture varied lies: flat, up/downhill, sidehill, bunker, divots/bare dirt, wet grass, ball on a tee vs on the ground.
- Safety: before every swing, make sure no one is in front or to the side; don't film in front of someone about to hit.
- Empty-scene negatives: flagstick, sprinkler heads, rakes, golf cart.
...
- **Hand over the raw files directly — no editing, no compression — and keep a backup.**
